...
// 追加精算（ticket_charges）の統合テスト。実 MySQL に対してセッション認証込みで
...
func TestIntegrationTicketChargeLifecycle(t *testing.T) {
	db := testdb.Open(t)
	...
	createReq := connect.NewRequest(&nazobuv1.CreateTicketChargeRequest{
		TicketId: ticketID,
		Title:    "打ち上げ飲み会",
		Participants: []*nazobuv1.TicketChargeParticipantInput{
			{UserId: organizerID, Amount: 3000},
			{UserId: memberID, Amount: 4500}, // 多めに飲食した人
		},
	})
	setSessionCookie(t, db, createReq, organizerID)
	if _, err := svc.CreateTicketCharge(ctx, createReq); err != nil {
		t.Fatalf("CreateTicketCharge に失敗: %v", err)
	}

	// GetTicket に charge が対象者・負担額つきで載ること
	detail := mustGetTicket(t, db, ticketID, organizerID)
	if len(detail.Charges) != 1 {
		t.Fatalf("charges 数 = %d, want 1", len(detail.Charges))
	}
	charge := detail.Charges[0]
	if charge.Title != "打ち上げ飲み会" {
		t.Errorf("Title = %q, want %q", charge.Title, "打ち上げ飲み会")
	}
	if charge.PaidByUserId != organizerID || charge.PayerName != "organizer-user" {
		t.Errorf("立替者 = %q(%q), want organizer", charge.PaidByUserId, charge.PayerName)
	}
	if !charge.CanEdit {
		t.Error("立替者本人の CanEdit が false")
	}
	if len(charge.Participants) != 2 {
		t.Fatalf("対象者数 = %d, want 2", len(charge.Participants))
	}
	byUser := make(map[string]*nazobuv1.TicketChargeParticipant, len(charge.Participants))
	for _, p := range charge.Participants {
		byUser[p.UserId] = p
	}
	...
	// 対象者（非立替者）から見ると charge は編集不可、charge 追加は参加者なので可
	memberView := mustGetTicket(t, db, ticketID, memberID)
	if memberView.Charges[0].CanEdit {
		t.Error("非立替者の charge CanEdit が true になっている")
	}
	if !memberView.CanAddCharge {
		t.Error("チケット参加者の CanAddCharge が false になっている")
	}

	// 精算トグル: 立替者が member を精算済みにできる
	settleReq := connect.NewRequest(&nazobuv1.UpdateTicketChargeSettlementRequest{
		ChargeId: charge.Id,
		UserId:   memberID,
		Settled:  true,
	})
	setSessionCookie(t, db, settleReq, organizerID)
	if _, err := svc.UpdateTicketChargeSettlement(ctx, settleReq); err != nil {
		t.Fatalf("UpdateTicketChargeSettlement に失敗: %v", err)
	}
	detail = mustGetTicket(t, db, ticketID, organizerID)
	for _, p := range detail.Charges[0].Participants {
	...
	updateReq := connect.NewRequest(&nazobuv1.UpdateTicketChargeRequest{
		ChargeId: charge.Id,
		Title:    "二次会",
		Participants: []*nazobuv1.TicketChargeParticipantInput{
	...
	if _, err := svc.UpdateTicketCharge(ctx, updateReq); err != nil {
		t.Fatalf("UpdateTicketCharge に失敗: %v", err)
	}
	detail = mustGetTicket(t, db, ticketID, organizerID)
	charge = detail.Charges[0]
	if charge.Title != "二次会" {
		t.Errorf("更新後 Title = %q, want %q", charge.Title, "二次会")
	}
	if len(charge.Participants) != 2 {
		t.Fatalf("更新後の対象者数 = %d, want 2", len(charge.Participants))
	}
	byUser = make(map[string]*nazobuv1.TicketChargeParticipant, len(charge.Participants))
	for _, p := range charge.Participants {
		byUser[p.UserId] = p
	...
	deleteReq := connect.NewRequest(&nazobuv1.DeleteTicketChargeRequest{ChargeId: charge.Id})
	setSessionCookie(t, db, deleteReq, organizerID)
	if _, err := svc.DeleteTicketCharge(ctx, deleteReq); err != nil {
		t.Fatalf("DeleteTicketCharge に失敗: %v", err)
	}
	detail = mustGetTicket(t, db, ticketID, organizerID)
	if len(detail.Charges) != 0 {
		t.Errorf("削除後も charges が残っている: %d 件", len(detail.Charges))
	}
}

func TestIntegrationTicketChargeValidationAndAuthorization(t *testing.T) {
	db := testdb.Open(t)
	ctx := context.Background()
	svc := newTicketService(db)
	...
	newCreateReq := func(participants []*nazobuv1.TicketChargeParticipantInput) *connect.Request[nazobuv1.CreateTicketChargeRequest] {
		return connect.NewRequest(&nazobuv1.CreateTicketChargeRequest{
			TicketId:     ticketID,
			Title:        "飲み会",
			Participants: participants,
		})
	}
	validParticipants := []*nazobuv1.TicketChargeParticipantInput{{UserId: participantID, Amount: 3000}}

	t.Run("session cookie なしは Unauthenticated", func(t *testing.T) {
		_, err := svc.CreateTicketCharge(ctx, newCreateReq(validParticipants))
	...
	t.Run("チケット非参加者の登録は PermissionDenied", func(t *testing.T) {
		...
		_, err := svc.CreateTicketCharge(ctx, req)
	...
	t.Run("admin はチケット非参加者でも登録できる", func(t *testing.T) {
		...
		if _, err := svc.CreateTicketCharge(ctx, req); err != nil {
			t.Errorf("admin の登録に失敗: %v", err)
		}
	})

	t.Run("チケット非参加者を対象にすると FailedPrecondition", func(t *testing.T) {
		req := newCreateReq([]*nazobuv1.TicketChargeParticipantInput{{UserId: outsiderID, Amount: 3000}})
	...
	t.Run("入力バリデーションは InvalidArgument", func(t *testing.T) {
		cases := map[string]*connect.Request[nazobuv1.CreateTicketChargeRequest]{
			"title なし": connect.NewRequest(&nazobuv1.CreateTicketChargeRequest{
				TicketId:     ticketID,
				Participants: validParticipants,
			}),
			"対象者なし": newCreateReq(nil),
			"対象者の重複": newCreateReq([]*nazobuv1.TicketChargeParticipantInput{
				{UserId: participantID, Amount: 3000},
				{UserId: participantID, Amount: 4000},
			}),
			"負の金額": newCreateReq([]*nazobuv1.TicketChargeParticipantInput{
	...
	// 以降は participant が立て替えた charge を使う
	createReq := newCreateReq([]*nazobuv1.TicketChargeParticipantInput{
		{UserId: participantID, Amount: 3000},
	...
	if chargeID == "" {
		t.Fatal("participant 立替の charge が見つからない")
	}

	t.Run("立替者でも admin でもない編集は PermissionDenied", func(t *testing.T) {
		req := connect.NewRequest(&nazobuv1.UpdateTicketChargeRequest{
			ChargeId:     chargeID,
			Title:        "改名",
			Participants: validParticipants,
		})
		setSessionCookie(t, db, req, outsiderID)
		_, err := svc.UpdateTicketCharge(ctx, req)
		if got := connectCode(t, err); got != connect.CodePermissionDenied {
			t.Errorf("code = %v, want %v", got, connect.CodePermissionDenied)
		}
	})
	...
	t.Run("立替者本人への精算トグルは FailedPrecondition", func(t *testing.T) {
		req := connect.NewRequest(&nazobuv1.UpdateTicketChargeSettlementRequest{
			ChargeId: chargeID,
			UserId:   participantID,
			Settled:  true,
		})
		setSessionCookie(t, db, req, participantID)
		_, err := svc.UpdateTicketChargeSettlement(ctx, req)
		if got := connectCode(t, err); got != connect.CodeFailedPrecondition {
			t.Errorf("code = %v, want %v", got, connect.CodeFailedPrecondition)
		}
	})
	...
	t.Run("存在しない charge は NotFound", func(t *testing.T) {
		req := connect.NewRequest(&nazobuv1.DeleteTicketChargeRequest{ChargeId: "00000000-0000-0000-0000-000000000000"})
		setSessionCookie(t, db, req, adminID)
		_, err := svc.DeleteTicketCharge(ctx, req)
		if got := connectCode(t, err); got != connect.CodeNotFound {
			t.Errorf("code = %v, want %v", got, connect.CodeNotFound)
		}
	})
...
func TestIntegrationTicketChargeMyPage(t *testing.T) {
	db := testdb.Open(t)
	ctx := context.Background()
	svc := newTicketService(db)
	...
	if got := listUnsettled(t, memberID); len(got) != 0 {
		t.Errorf("charge 登録前の member の未精算 = %v, want 空", got)
	}
	if got := listReceivables(t, adminID); len(got) != 0 {
		t.Errorf("charge 登録前の admin の未回収 = %v, want 空", got)
	}
	if !monthlySettled(t, memberID) {
		t.Error("charge 登録前の月次履歴が未精算になっている")
	}
	...
	chargeID := mustGetTicket(t, db, ticketID, organizerID).Charges[0].Id
	...
	if got := listUnsettled(t, organizerID); len(got) != 0 {
		t.Errorf("幹事（charge 立替者）の未精算 = %v, want 空", got)
	}
	if got := listReceivables(t, adminID); len(got) != 0 {
		t.Errorf("admin の未回収 = %v, want 空", got)
	}
	if monthlySettled(t, memberID) {
		t.Error("charge 未精算なのに月次履歴が精算済みになっている")
	}
